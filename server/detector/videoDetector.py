...
class VideoDetector:
    ...
    @staticmethod
    def intersects(contour, rect):
        x1, y1, x2, y2 = rect
        if cv.pointPolygonTest(contour, (x1, y1), False) >= 0:
            return True
        if cv.pointPolygonTest(contour, (x1 + x2, y1), False) >= 0:
            return True
        if cv.pointPolygonTest(contour, (x1, y1 + y2), False) >= 0:
            return True
        if cv.pointPolygonTest(contour, (x1 + x2, y1 + y2), False) >= 0:
            return True
        return False
